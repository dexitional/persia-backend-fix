...
module.exports.SSO = {
  ...
  postEvsData: async (data) => {
    ...
    try {
      // Get Election Data
      var resp = await db.query("select v.tag,e.id as eid,e.tag,e.voters_count,e.voters_whitedata,e.live_status,e.status,v.vote_time,v.vote_status,v.vote_sum,JSON_SEARCH(e.voters_whitelist, 'one', '"+tag+"') as voter from ehub_vote.election e left join ehub_vote.elector v on (e.id = v.election_id and v.tag = '"+tag+"') where json_search(e.voters_whitelist, 'one', '"+tag+"') is not null and e.live_status = 1 and e.id = "+id);
